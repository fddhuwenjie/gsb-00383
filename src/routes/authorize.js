...
function createAuthorizeRoute({ config, data, grantService }) {
...
`;
  ...
  router.get('/authorize', (req, res) => {
    ...
    const client = data.getClientById(client_id);
    if (!client) {
      return res.status(400).json({
  ...
  router.post('/authorize', express.urlencoded({ extended: true }), (req, res) => {
    ...
    const valid = data.verifyUserPassword(username, password);
    if (!valid) {
...
module.exports = createAuthorizeRoute;
