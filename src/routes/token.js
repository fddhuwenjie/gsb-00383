...
function createTokenRoute({ config, data, jwt, grantService }) {
  ...
  router.post('/token', express.urlencoded({ extended: true }), async (req, res) => {
    ...
    const client = data.getClientById(clientAuth.client_id);
    if (!client) {
      return res.status(401).json({
        error: 'invalid_client',
    ...
    if (client.client_type === 'confidential') {
      if (!data.verifyClientCredentials(clientAuth.client_id, clientAuth.client_secret)) {
        return res.status(401).json({
          error: 'invalid_client',
  ...
  async function handleAuthorizationCodeGrant(req, res, client, code, redirect_uri, code_verifier) {
    ...
    const consumption = grantService.consumeAuthorizationCode({
      code,
    ...
    if (!consumption.ok) {
      return res.status(consumption.status).json({
        error: consumption.error,
        error_description: consumption.error_description
      });
    }

    const authCode = consumption.authCode;

    const user = data.getUserById(authCode.user_id);
    if (!user) {
      return res.status(400).json({
        error: 'server_error',
    ...
    const accessToken = await jwt.signAccessToken(accessTokenPayload);

    const { refreshTokenValue } = grantService.issueTokenPair({
      clientId: client.client_id,
      userId: user.id,
      scope: authCode.scope,
      accessToken
  ...
  async function handleRefreshTokenGrant(req, res, client, refreshTokenValue, scope) {
    ...
    const consumption = grantService.consumeRefreshToken({
      refreshTokenValue,
      clientId: client.client_id,
    ...
    const newScope = consumption.newScope;

    const user = data.getUserById(consumption.userId);
    if (!user) {
      return res.status(400).json({
        error: 'server_error',
        error_description: 'User not found'
    ...
    const newAccessToken = await jwt.signAccessToken(accessTokenPayload);

    const { refreshTokenValue: newRefreshTokenValue } = grantService.issueTokenPair({
      clientId: client.client_id,
      userId: user.id,
      scope: newScope,
  ...
  router.post('/token/downscope', express.urlencoded({ extended: true }), async (req, res) => {
    ...
    const jwtResult = await jwt.verifyJwt(access_token);
    if (!jwtResult.valid) {
      return res.status(400).json({
        error: 'invalid_token',
    ...
    const tokenRecord = data.getToken(access_token);
    ...
    const newAccessToken = await jwt.signAccessToken(newPayload, newExpiresIn);

    data.storeToken('access_token', newAccessToken, client.client_id, tokenRecord.user_id, scope,
